...
def get_recognition_TCN(data, ml_model):
    	
    #Create dataset from list of data
    gen=get_test(data)
    pred_label=[]
    probabilities=[]
...
def get_training(data_training, label_training, epoch):
    ...
    """Function to get a training dataset needed for TCN model.
    :param data_training: data_training set composed by a list of events
    :param label_training : label set composed by a set of labels
    :param epoch: number of training epoch 
    :returns: dataset structure created for a specific number of epoch
    """ 
...
def get_test(data_training):

    """Function to get a test dataset needed for TCN model.
    :param data_training: data_test set composed by a list of events
    :param label_training : label set composed by a set of labels 
...
def Retraining_model_by_station(model, dataset, labels, original_dataset, original_labels, namefile):
    ...
    model.compile(optimizer='sgd', loss=tf.losses.CategoricalCrossentropy(),metrics=['accuracy'])

    for iteration in range (training_iters):
        #data_training, label_training_popo_confidence=create_Data_Base(m, data_training_popo, label_training_popo, 0.5)
        gen=get_training(data_training, label_training, epoch)
        model.fit(gen, epochs=epoch, steps_per_epoch=len(data_training), max_queue_size=1, verbose=2, batch_size=1)

		#Test los datos del Popo
        gen=get_test(data_test)
        ...
        print (confusion_matrix(label_test, pred_label)) 
        print ('Overall accuracy:', accuracy_score(label_test, pred_label))
    
    
    pred_label_after_weakly, probabilities_after_weakly=get_recognition_TCN(original_dataset,model)
...
def create_Data_Base_Using_Grammar(ml_model, dataset, labels, prob_threshold):
    ...
    pred_label, probabilities=get_recognition_TCN(dataset,ml_model)
...
def create_Data_Base(m, data_training_popo, label_training_popo, prob_threshold):
	pred_label=[]
	probabilities=[]
	data_training_popo_confidence=[]
	label_training_popo_confidence=[]

	gen=get_training(data_training_popo, label_training_popo, 1)
	for i,item in enumerate (gen):
		data, lab =item
	...
	events_delimited=Event_Limits(pred_label)
	for i,records in enumerate(events_delimited):
		down=0
		suggested_label=[]
		threshold=[]
		...
		if (not False in threshold):
			data_training_popo_confidence.append(data_training_popo[i])
			label_training_popo_confidence.append(label_training_popo[i])
			down_label=0
			for k,index in enumerate (records):
				label_training_popo_confidence[-1][down_label:index]=suggested_label[k]
				down_label=index
			label_training_popo_confidence[-1][down_label:]=suggested_label[-1]

	print ('New dataset size is: ', len(data_training_popo_confidence))
	print ('Original dataset size was: ', len(data_training_popo))
	return data_training_popo_confidence, label_training_popo_confidence
...
def process(station_path, station_name, dates,files_coherence, model ,ml_model):
    ...
    detection_, probabilities_= get_recognition_TCN(dataset,ml_model)
...
def process_retraining(station_path,min_chunk,prob_threshold,station_name, dates,model, ml_model):
    ...
    datatraining_confidence, label_confidence, chunk_selected= create_Data_Base_Using_Grammar(ml_model, dataset,labels, prob_threshold)
    ...
    Retraining_model_by_station(ml_model, datatraining_confidence, label_confidence, dataset, labels, f'{aux}{station_name}{model}{date}{extention}')
    end_time = time.perf_counter()
    print("Elapsed time for weakly retraining: ", elapsed_time)

def real_time_monitoring_TCN_parallel(station_path, station_names, coherence_event, dates, model, expert_by_station):
...
def real_time_monitoring_TCN(station_path, station_names, coherence_event, dates, model ,ml_model):
...
def read_Config_File_TCN():
    
    reading=[]
    station_path=[]
    config = configparser.ConfigParser()
    config.read('../config_real_time.ini')
    num_filters=int(config['DEFAULT']['tcn_n_filters'])
    ...
    model_path='..'+config['model_path']['path_tcn']
    ...
    return num_filters, num_station, station_names,coherence_event, reading, frequency, model_path, real_time, dates, model

def read_Config_File_TCN_parallel(aux_file):
    ...
    return  num_filters,num_station, station_names,coherence_event, reading, frequency, model_paths, real_time, dates, model

def read_Config_File_TCN_weakly(aux_file):
    ...
    return  num_filters,num_station, weakly, min_chunk, prob_threshold,station_names,coherence_event, reading, frequency, model_paths, real_time, dates, model